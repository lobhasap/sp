...
void processMacro(char *name, char params[MAX_PARAMS][MAX_NAME_LENGTH], int paramCount, FILE *inputFile) {
...
void displayTables() {
    ...
    for (int i = 0; i < mnt_count; i++) {
        printf("Name: %s, MDT Index: %d, Params: ", mnt[i].name, mnt[i].mdt_index);
        for (int j = 0; j < mnt[i].param_count; j++) {
            printf("%s ", mnt[i].params[j]);
        }
        printf("\n");
    }
...
int main() {
    FILE *inputFile = fopen("macdef.txt", "r");
    ...
    while (fgets(line, sizeof(line), inputFile)) {
        if (strncmp(line, "MACRO", 5) == 0) {
            ...
            processMacro(macro_name, params, paramCount, inputFile);
        }
    }

    displayTables();  // Output is displayed in terminal now
    fclose(inputFile);
